...
export class GeminiService implements OnModuleDestroy {
  ...
  async chat(
    ...
    // Filter tools and convert to Gemini format
    const filteredTools = this.intentRouter.getFilteredTools(user, companyType, isWeb);
      ...
      while (loopCount < MAX_TOOL_LOOPS) {
        ...
        if (selectedModel === GEMINI_MODELS.flash) {
          ...
          if (prepareCall) {
            ...
            if (hasMissingFields && !this._proRetried?.has(session.id)) {
