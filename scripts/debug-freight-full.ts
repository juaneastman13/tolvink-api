...
async function main() {
  ...
  if (!f) { console.log(`${code} not found`); return; }
  ...
  console.log('\n--- Assignments ---');
  if (f.assignments.length === 0) console.log('  (none)');
  f.assignments.forEach(a => {
    console.log(`  id:${a.id} status:${a.status} transport:${a.transportCompany?.name || 'N/A'} truck:${a.truck?.plate || a.plate || 'N/A'} driver:${a.driver?.name || 'N/A'} tripStatus:${(a as any).tripStatus || 'N/A'}`);
  });
