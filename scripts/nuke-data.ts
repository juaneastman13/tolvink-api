// =====================================================================
// TOLVINK — Nuke all data EXCEPT the admin user
// Usage: npx ts-node scripts/nuke-data.ts [admin-email]
// Default admin: keeps the user with is_super_admin=true
// =====================================================================
...
/** Safe deleteMany — skips if table doesn't exist in DB */
async function del(model: any, label: string, where?: any) {
  try {
...
async function main() {
  const adminEmail = process.argv[2]; // optional: specific email to keep
  ...
  // --- Freight children (deepest first) ---
  ...
  // --- Freights (after all children removed) ---
  ...
  // Clear admin's company references before deleting companies
  ...
  // --- Companies (after all FK refs removed) ---
